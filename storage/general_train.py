...
def main(args):
    ...
    args.data_cats = {k: v['type'] for k, v in args.task_cfg.items() if v is not None}
    ...
    if args.validate:
        ...
        results = evaluate.evaluate(model, val_loaders, args.data_cats,
                           logger, args.use_origin)
    ...
    assert (not args.step_train and args.general) \
        or (args.step_train and not args.general)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        time.sleep(2)
        results = evaluate.evaluateresults = evaluate.evaluate(model, val_loaders, args.data_cats, logger)
