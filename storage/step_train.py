...
from lib.models.model_lib import step_model, general_model
...
def main(args):
    ...
    # model_fn = step_model
    # backbone = model_fn.SKBackbone(
    #     args.backbone, 
    #     args.detector,
    #     args.segmentor, 
    #     backbone_weight=args.state_dict['backbone'],
    #     freeze_backbone=args.freeze_backbone,
    #     **kwargs)
    
    # backbone_out_channels = backbone.backbone.body_out_channel
    # fpn_out_channels = backbone.backbone.fpn_out_channels
    # clf_stem = model_fn.SKClfStem()
    # clf_head = model_fn.SKClfHead(backbone_out_channels, args.task_cfg['clf']['num_classes'])
    
    # det_stem = model_fn.SKDetStem(stem_weight=args.state_dict['stem'])
    # det_head = model_fn.SKDetHead(args.detector, fpn_out_channels,
    #                               args.task_cfg['det']['num_classes'])
    
    # seg_stem = model_fn.SKSegStem(stem_weight=args.state_dict['stem'])
    # seg_head = model_fn.SKSegHead(args.segmentor, backbone_out_channels,
    #                               args.task_cfg['seg']['num_classes'])
    
    # if args.distributed:
    #     backbone_group = dist.new_group(ranks=[0, 1])
    #     clf_stem_group = dist.new_group(ranks=[2])
    #     clf_head_group = dist.new_group(ranks=[3])
    #     det_stem_group = dist.new_group(ranks=[4])
    #     det_head_group = dist.new_group(ranks=[5])
    #     seg_stem_group = dist.new_group(ranks=[6])
    #     seg_head_group = dist.new_group(ranks=[7])
        
    #     # backbone = torch.nn.parallel.DistributedDataParallel(
    #     #     backbone, device_ids=[0, 1], process_group=backbone_group)
    #     # clf_stem = torch.nn.parallel.DistributedDataParallel(
    #     #     backbone, device_ids=[2], process_group=clf_stem_group)
    #     # clf_head = torch.nn.parallel.DistributedDataParallel(
    #     #     backbone, device_ids=[3], process_group=clf_head_group)
    #     # det_stem = torch.nn.parallel.DistributedDataParallel(
    #     #     backbone, device_ids=[4], process_group=det_stem_group)
    #     # det_head = torch.nn.parallel.DistributedDataParallel(
    #     #     backbone, device_ids=[5], process_group=det_head_group)
    #     # seg_stem = torch.nn.parallel.DistributedDataParallel(
    #     #     backbone, device_ids=[6], process_group=seg_stem_group)
    #     # seg_head = torch.nn.parallel.DistributedDataParallel(
    #     #     backbone, device_ids=[7], process_group=seg_head_group)
        
    #     backbone = torch.nn.parallel.DistributedDataParallel(
    #         backbone, device_ids=[args.gpu], process_group=backbone_group)
    #     clf_stem = torch.nn.parallel.DistributedDataParallel(
    #         backbone, device_ids=[args.gpu], process_group=clf_stem_group)
    #     clf_head = torch.nn.parallel.DistributedDataParallel(
    #         backbone, device_ids=[args.gpu], process_group=clf_head_group)
    #     det_stem = torch.nn.parallel.DistributedDataParallel(
    #         backbone, device_ids=[args.gpu], process_group=det_stem_group)
    #     det_head = torch.nn.parallel.DistributedDataParallel(
    #         backbone, device_ids=[args.gpu], process_group=det_head_group)
    #     seg_stem = torch.nn.parallel.DistributedDataParallel(
    #         backbone, device_ids=[args.gpu], process_group=seg_stem_group)
    #     seg_head = torch.nn.parallel.DistributedDataParallel(
    #         backbone, device_ids=[args.gpu], process_group=seg_head_group)
