...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=8, patch_size=2, in_chans=2048, num_classes=10, embed_dims=[64, 128, 256, 512],
                 num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm, depths=[3, 4, 6, 3],
                 sr_ratios=[8, 4, 2, 1], F4=False, num_stages=4, pretrained=None):
        super().__init__()
...
class pvt_tiny_f4(PyramidVisionTransformer):
    def __init__(self, **kwargs):
        super(pvt_tiny_f4, self).__init__(
            patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4],
            qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2],
...
# pe = PatchEmbed(img_size=8, patch_size=2, in_chans=2048, embed_dim=2048)
# data = torch.rand(1, 2048, 8, 8)
# out = pe(data)
