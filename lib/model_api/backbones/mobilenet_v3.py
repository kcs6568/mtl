...
class MobileNetV3(nn.Module):
    ...
    def __init__(
        ...
        self.last_out_channel = lastconv_output_channels
...
def _mobilenet_v3_conf(arch: str, width_mult: float = 1.0, reduced_tail: bool = False, dilated: bool = False,
    ...
    if arch == "mobilenet_v3_large":
        if kwargs['no_st_early']:
            early_st = 1
        else:
            early_st = 2
            
        # print(dilated, dilation)
        # print(early_st)
        # exit()
        
        inverted_residual_setting = [
            bneck_conf(16, 3, 16, 16, False, "RE", 1, 1),
            bneck_conf(16, 3, 64, 24, False, "RE", early_st, 1),  # C1
            # bneck_conf(16, 3, 64, 24, False, "RE", 2, 1),  # C1
            # bneck_conf(16, 3, 64, 24, False, "RE", 1, 1),  # C1 --> no downsampling 
...
def get_mobilenet_v3(model, weight_path=None, pretrained=True, **kwargs):
    if 'large' in model:
        return mobilenet_v3_large(pretrained=pretrained, weight_path=weight_path, **kwargs)
    elif 'small' in model:
        return mobilenet_v3_small(pretrained=pretrained, weight_path=weight_path, **kwargs)


def mobilenet_v3_large(pretrained: bool = False, weight_path=None, progress: bool = True, **kwargs: Any) -> MobileNetV3:
    """
    ...
    return _mobilenet_v3_model(arch, inverted_residual_setting, last_channel, pretrained, progress, weight_path, **kwargs)


def mobilenet_v3_small(pretrained: bool = False, weight_path=None, progress: bool = True, **kwargs: Any) -> MobileNetV3:
    """
...
def setting_mobilenet_args(args, model_args):
    if 'no_st_early' in args:
        model_args.update({'no_st_early': args.no_st_early})
    
    if 'dilated' in args:
        model_args.update({'dilated': args.dilated})
    else:
        model_args.update({'dilated': False})
