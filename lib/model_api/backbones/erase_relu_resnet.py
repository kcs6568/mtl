...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = conv3x3(width, width, stride, groups, dilation, deform)
...
class ResNet(nn.Module):
    def __init__(
        ...
        if deform_layers is None:
            deform_layers = [False for _ in range(len(layers))]
        # assert len(layers) == len(deform_layers)
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        ...
        self.layer1 = self._make_layer(block, self.in_channels[0], layers[0], deform=deform_layers[0])
        self.layer2 = self._make_layer(block, self.in_channels[1], layers[1], stride=2,
                                       dilate=replace_stride_with_dilation[0], deform=deform_layers[1])
        self.layer3 = self._make_layer(block, self.in_channels[2], layers[2], stride=2,
                                       dilate=replace_stride_with_dilation[1], deform=deform_layers[2])
        self.layer4 = self._make_layer(block, self.in_channels[3], layers[3], stride=2,
                                       dilate=replace_stride_with_dilation[2], deform=deform_layers[3])
    ...
    def _make_layer(self, block: Type[Union[BasicBlock, Bottleneck]], planes: int, blocks: int,
        ...
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups,
                            self.base_width, previous_dilation, norm_layer, deform))
        self.inplanes = planes * block.expansion
        for _ in range(1, blocks):
            layers.append(block(self.inplanes, planes, groups=self.groups,
                                base_width=self.base_width, dilation=self.dilation,
                                norm_layer=norm_layer, deform=deform))
...
args = {'deform_layers': [False, True, True, True]}

model = get_resnet('resnet50', **args)
