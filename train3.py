...
def main(args):
    ...
    if args.resume or args.resume_tmp or args.resume_file:
        ...
        try:
            ...
            if args.lr_scheduler == 'step':
                ...
                optimizer.param_groups[0]['lr'] = checkpoint['lr_scheduler']['_last_lr']
    ...
    if args.make_cm:
        logger.log_text("Only make confusion matrix for classification")    
        engines.classification_for_cm(model, val_loaders, args.data_cats, args.output_dir)
        logger.log_text("Finish generation CM\n\n")
    ...
    if args.validate:
        ...
        if args.only_val:
            logger.log_text("Process terminate after only validation\n")
            import sys
            sys.exit()
    ...
    if get_rank() == 0:
        tmp_path = os.path.join(args.output_dir, 'ckpts', 'tmp_checkpoint.pth')
